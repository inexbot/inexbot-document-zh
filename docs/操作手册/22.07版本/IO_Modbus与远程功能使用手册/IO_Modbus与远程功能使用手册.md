...
1. [IO](#1-io)
   - [1.1 输入输出指令](#11-输入输出指令)
     - [1.1.1 DIN-IO输入](#111-din-io输入)
     - [1.1.2 DOUT-IO输出](#112-dout-io输出)
     - [1.1.3 AIN-模拟输入](#113-ain-模拟输入)
     - [1.1.4 AOUT-模拟输出](#114-aout-模拟输出)
     - [1.1.5 PULSEOUT-脉冲输出](#115-pulseout-脉冲输出)
     - [1.1.6 READ_DOUT-读取输出](#116-read_dout-读取输出)
2. [IO状态提示设置](#2-io状态提示设置)
3. [IO安全设置](#3-io安全设置)
   - [3.1 IO复位](#31-io复位)
   - [3.2 IO配置](#32-io配置)
   - [3.3 使能IO](#33-使能io)
   - [3.4 报警消息](#34-报警消息)
   - [3.5 端口名称](#35-端口名称)
4. [远程模式IO预约简要说明](#4-远程模式io预约简要说明)
   - [4.1 远程模式状态说明](#41-远程模式状态说明)
   - [4.2 远程模式控制权说明](#42-远程模式控制权说明)
   - [4.3 远程IO控制](#43-远程io控制)
     - [4.3.1 远程I/O功能选择设置](#431-远程io功能选择设置)
     - [4.3.2 远程程序设置](#432-远程程序设置)
     - [4.3.3 预约模式](#433-预约模式)
5. [远程功能的使用](#5-远程功能的使用)
   - [5.1 远程功能概述](#51-远程功能概述)
6. [MODBUS](#6-modbus)
   - [6.1 Modbus指令](#61-modbus指令)
   - [6.2 Modbus从站](#62-modbus从站)
   - [6.3 Modbus主站](#63-modbus主站)
   - [6.4 Modbus修改地址码](#64-modbus修改地址码)
   - [6.5 Modbus的使用](#65-modbus的使用)
     - [6.5.1 Modbus poll连接方式](#651-modbus-poll连接方式)
     - [6.5.2 任何模式下Modbus可读取全局位置](#652-任何模式下modbus可读取全局位置)
     - [6.5.3 Modbus多主站连接](#653-modbus多主站连接)
     - [6.5.4 Modbus与IO优先级](#654-modbus与io优先级)
     - [6.5.5 Modbus触摸屏使用流程](#655-modbus触摸屏使用流程)
     - [6.5.6 Modbus参数说明](#656-modbus参数说明)
...
![controller.json配置](assets/image3.png)
...
![状态提示设置界面1](assets/image4.png)

![状态提示设置界面2](assets/image5.png)
...
![报警消息设置示例1](assets/image12.png)

![报警消息设置示例2](assets/image13.png)
...
![端口名称设置界面](assets/image14.png)

**使用示例：**

如设置DIN1-1 名称为"使能端口"，则在【状态】的【输入输出】中，会显示DIN1名称"使能端口"。

![端口名称显示](assets/image15.png)
...
![远程IO功能设置界面1](assets/image17.png)

![远程IO功能设置界面2](assets/image18.png)
...
![Modbus读操作示例1](assets/image21.png)
...
![Modbus读操作示例2](assets/image22.png)

![Modbus读操作示例3](assets/image24.png)
...
![Modbus 3x-bit操作示例1](assets/image25.png)

![Modbus 3x-bit操作示例2](assets/image26.png)

![Modbus 3x-bit操作示例3](assets/image27.png)
...
![Modbus写操作示例1](assets/image28.png)
...
![Modbus写操作示例2](assets/image29.png)
...
![Modbus从站设置界面](assets/image30.png)
...
![Modbus主站TCP设置界面](assets/image31.png)

![Modbus主站RTU设置界面](assets/image32.png)
...
![配置文件目录](assets/image33.png)

3. **使用Notepad++等文本编辑软件打开**

![modbusAddr.json文件内容](assets/image34.png)

4. **打开后**可以看到一个{ }中包含一组地址码参数（系统会自动生成一份原始地址码）

![地址码参数结构](assets/image35.png)

5. **修改地址**直接更改addr后的数字，数字为0时，该地址码功能无效

![修改地址码示例](assets/image36.png)

6. **修改完成后点击保存**

![保存修改](assets/image37.png)

7. **然后把该参数重新导入控制器**，重启生效

![导入配置](assets/image38.png)

8. **修改参数后重启或者重新打开连接后生效**（导入配置文件会自动重启）

![重启后生效](assets/image39.png)
...
![Modbus参数设置](assets/image40.png)

2. **在设置/modbus设置/modbus程序界面**，选择程序

![Modbus程序选择](assets/image41.png)
...
![Modbus Poll连接](assets/image42.png)

![Connection Setup对话框](assets/image43.png)

5. **设置Setup→Read/Write Definition参数**（图片填写参数仅作示例）：如遇填写地址码不生效可排查：将起始地址改为1，勾选PLC Addresses选项，点击OK

![Read/Write Definition对话框](assets/image44.png)

6. **双击寄存器数据**，填写相关地址码

![寄存器数据填写](assets/image45.png)

![Write Single Register对话框](assets/image46.png)
...
![地址2004设置](assets/image47.png)

2. **根据Modbus地址2000**选择读取全局点位的编号，编号为1-999

![地址2000设置](assets/image48.png)

3. **将2017-2028和2031-2036的Format改为FloatCD AB**

![Format设置](assets/image49.png)
...
![Modbus多主站设置](assets/image50.png)
...
![ModbusAddr.json配置](assets/image51.png)
...
![触摸屏设备属性设置](assets/image54.png)

触摸屏程序编辑好后运行，示教盒远程界面modbus未连接变为modbus已连接。

![远程模式Modbus连接状态](assets/image55.jpeg)
