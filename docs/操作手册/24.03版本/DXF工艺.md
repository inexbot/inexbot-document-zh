...
A1: DXF功能支持读取直线、圆弧、多段线、样条曲线、整圆图元，其它图元暂不支持。
...
A3: 离散点仅对样条曲线生效，离散点越高，生成的作业文件点位越多。其他支持的图元无需修改离散点，使用默认离散点即可，离散点值对其无效。目前支持离散点数值最高为5000。
...
A8: 可在程序第一行或最后一行添加一个关节坐标下的点到点指令作为安全点，安全点设置好后需要运行看一下轨迹是否正确，若安全点设置不合理会导致关节超限，运行轨迹异常。

**Q9: 为什么生成的作业文件运行轨迹与预期不符？**
