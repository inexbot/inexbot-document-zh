...
A: FINSTCP功能仅适用于欧姆龙PLC，不支持其他品牌的PLC。

**Q: 如何判断控制器与PLC是否连接成功?**

A: 当控制器与PLC通讯连接上后，全局布尔型变量中第10个变量的数值会在1与0之间不断跳动，说明连接成功。

**Q: 为什么PLC的写入地址与读取地址不能设置成一样的数值?**

A: 如果设置成相同的数值，会导致数据冲突，影响通讯的正常进行。

**Q: 如何通过PLC控制控制器的运行?**

A: 在控制器读取地址中的全局布尔型变量中，第4个变量PLC写入数值为1时，控制器主程序开始运行（控制器需要处于运行模式）。

**Q: 如何通过PLC清除控制器的报警?**

A: 在控制器读取地址中的全局布尔型变量中，第7个变量PLC写入数值为1时，控制器的报警小白条被清除，与清错按钮的效果一致。

**Q: 为什么控制器读取地址中的变量不能通过示教器修改?**

A: 控制器读取地址中的变量是由PLC写入的，为了保证数据的一致性，这些变量不能通过示教器修改，只能通过PLC进行修改。
