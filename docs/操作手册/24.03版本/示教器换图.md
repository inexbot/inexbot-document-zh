---
title: "示教器换图"
description: "纳博特示教器（T30/PC版）图片更换操作指南，涵盖LOGO、启动图片、文字说明、二维码、左侧图标及内容区图标的更换方法与命名规范。"
author: "MUZI165"
date: "2026-04-03"
tags: ["示教器", "换图", "LOGO", "T30", "PC版", "图片更换", "命名规范"]
category: "操作手册"
version: "1.0.0"
language: "zh-CN"
---
...
2. Info文本编码格式必须为UTF-8。

3. 更换左侧图标：该功能仅支持22.07版本及以上版本。

4. PC版不需要升级，复制到程序安装目录下的change/img文件夹下。

5. T30操作：若要一次性将所有内容更换，可以将以上所提到的所有文件压缩在一个.zip 压缩包内，升级该文件即可。

6. 图片命名时注意英文大小写。

特殊说明：图片另存为时，当保存类型为PNG格式时，图片名不可以带后缀，如图：

![格式](./assets/jgiuhp2k15qz3pwin1awu.png)

当打开显示文件扩展名时，图片显示：

![显示扩展名](./assets/wmym8ozovrknhc2q3ka8l.png)

当关闭显示文件扩展名时，图片显示：

![关闭扩展名](./assets/qs7ssl78q5rrsgkqd28aq.png)
...
3. **T30操作：** 将.zip 压缩包放在 U 盘根目录下，插在示教器上，升级该文件；
...
![文字更改示例](./assets/idsp4tuodl2y5bohpd3bc.png)
...
3.  **T30操作：** 将 QR.zip 文件放在 U 盘根目录下，插在示教器上，升级该文件。
...
**T30操作：** 若要一次性将所有内容更换，可以将以上所提到的所有文件压缩在一个.zip 压缩包内，升级 该文件即可。
...
A：PNG 格式，例如 StartImage.png、SoftwareUpdatingBackground.png。二维码图片也需要是 PNG 格式，命名必须为 QR.png。

**Q：如何查看示教器是什么型号？**

A：示教器背面写有产品型号，如 T30、T30-NBT 等。请根据实际型号选择对应的操作方式（T30 使用 U 盘升级，PC 版直接复制文件）。

**Q：如何查看示教器版本？**

A：在"设置" - "系统设置" - "版本升级"中查看，示教器版本号格式为 rtl-xx.xx.xx-yyyyMMddHH，其中 xx.xx.xx 即为主版本号。

**Q：更换公司介绍后显示为乱码怎么办？**

A：公司介绍需使用 txt 文档，命名为 Info.txt，必须使用 UTF-8 编码格式，否则会出现乱码。

**Q：更换图片必须使用 .zip 压缩包吗？**

A：使用 T30 示教器进行更换时，需将要更换的内容压缩在一个 .zip 压缩包内（如 background.zip），通过 U 盘升级。使用 PC 版进行更换时，直接将文件复制到程序安装目录下的 change/img 文件夹下即可，无需压缩。

**Q：更换图片时需要注意什么？**

A：注意以下要点：图片命名时需要注意英文大小写；二维码压缩包需命名为 QR.zip；保存为 PNG 格式时，图片文件名本身不需要带 .png 后缀（例如保存时类型选 PNG，则文件名直接写 Logo.png 即可，不要写成 Logo.png.png）；T30 和 PC 版的图片命名规则不同，请注意区分。

**Q：使用 T30 示教器换图如何进行升级？**

A：将内容压缩包放入 U 盘中，将 U 盘插入示教器，在"设置" - "系统设置" - "版本升级"中点击"检测升级"即可。

**Q：PC 版和 T30 示教器的图片命名有什么区别？**

A：PC 版的程序启动图片命名为 windowsStartImage.png 和 SoftUpdatingBackgroundWindows.png（与 T30 不同）；左侧图标等大部分图片命名规则相同，均支持中、英、韩三种语言（通过 en/kr 前缀区分）。

**Q：更换左侧图标有什么限制？**

A：该功能仅支持 22.07 版本及以上版本。图片分辨率均为 92×53 像素，PNG 格式，命名需区分权限等级（管理员/技术员/操作员/自定义用户）和语言。分辨率错误将导致图标显示异常。

---

## 相关资源

- [示教器修改主题颜色功能教程](./示教器修改主题颜色功能教程.md)
